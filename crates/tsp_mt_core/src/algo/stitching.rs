...
fn merge_two_cycles_dense(coords: &[LKHNode], tour_a: &[usize], tour_b: &[usize]) -> MergeResult {
    ...
    for &(_, u_node, v_node) in candidates.iter().take(check_limit) {
        ...
        for (a1, a2) in a_cuts {
            for (b1, b2) in b_cuts {
                ...
                let penalty_fwd =
                    if e1 > LARGE_JUMP_DISTANCE_THRESHOLD || e2 > LARGE_JUMP_DISTANCE_THRESHOLD {
                        LARGE_JUMP_PENALTY
                    } else {
                        0.0
                    };
                ...
                let penalty_rev =
                    if r1 > LARGE_JUMP_DISTANCE_THRESHOLD || r2 > LARGE_JUMP_DISTANCE_THRESHOLD {
                        LARGE_JUMP_PENALTY
                    } else {
                        0.0
                    };
...
mod tests {
    ...
    fn merge_two_cycles_dense_keeps_all_nodes_exactly_once() {
        ...
        let result = merge_two_cycles_dense(&coords, &tour_a, &tour_b);
